...
contract User is SharingEconomy{
    ...
    //返回交易记录
    function getDealRec(uint id,uint deal)public view returns(uint,string memory){
        return(goodsData[id].dealData[deal].borrowDays,goodsData[id].dealData[deal].since);
    }
    //返回归还记录
    function getBackRec(uint id,uint back)public view returns(uint,uint,bytes32,uint,bool,string memory){
        return (goodsData[id].backData[back].blockNum,goodsData[id].backData[back].dealDate,goodsData[id].backData[back].signHash,goodsData[id].backData[back].borrowDays,goodsData[id].backData[back].isBackAgree,goodsData[id].backData[back].over);
    }
    ...
    //同意借用
    function agreeBorrow(uint id,uint deal,string memory since) public {
    ...
    //交易哈希
    function dealHash(uint id,uint back,uint blockNum) public{
        require(goodsData[id].isAgree=true,"disagree");
        goodsData[id].backData[back].signHash = blockhash(blockNum);
    }
    ...
    //同意归还
    function agreeBack(uint id,uint backs,string memory over) public payable returns(uint){
        require(goodsData[id].owner ==msg.sender,"owner isn't msg");
        ...
        orderData[orderId].back=backs;
        goodsData[id].backData[backs].blockNum = block.number;
        goodsData[id].backData[backs].dealDate = block.timestamp;
    ...
    //不同意归还
    function disagreeBack(uint id,uint backs,string memory over) public returns(uint){
        require(goodsData[id].owner ==msg.sender,"owner isn't msg");
