...
contract User is SharingEconomy {
    ...
    //返回交易记录
    function getDealRec(uint id, uint deal) public view returns (uint, uint, bytes32, uint){
        return (goodsData[id].dealData[deal].blockNum, goodsData[id].dealData[deal].dealDate, goodsData[id].dealData[deal].signHash, goodsData[id].dealData[deal].borrowDays);
    }


    //返回归还记录
    function getBackRec(uint id, uint back) public view returns (bool){
        return goodsData[id].backData[back].isBackAgree;
    }
    ...
    //  借用物品
    function borrowGoods(uint id, uint borrowDays) public {
        //物品必须存在
        require(goodsData[id].available == true,
            "goods not exist");
        ...
        payable(host).transfer(borrowDays * goodsData[id].rent + goodsData[id].ethPledge);
    }
    ...
    function agreeBorrow(uint id, uint deal, string memory since) public {
        ...
        payable(goodsData[id].owner).transfer(goodsData[id].dealData[deal].borrowDays * goodsData[id].rent);
    ...
    function disagreeBorrow(uint id, uint deal) public payable {
        require(goodsData[id].owner == msg.sender, "owner isn't msg");
        payable(goodsData[id].borrowers.borrower).transfer(goodsData[id].dealData[deal].borrowDays * goodsData[id].rent + goodsData[id].ethPledge);
        goodsData[id].isAgree = false;
        goodsData[id].borrowers.borrower = address(0);
    }

    //借出物品
    function borrow(uint id, uint deal) public {
        //设置为已借出
    ...
    //不同意归还
    function disagreeBack(uint id, uint backs) public {
        require(goodsData[id].owner == msg.sender, "owner isn't msg");
