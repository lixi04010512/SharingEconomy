...
//封装获取用户信息方法
func GetUserMethod(contract *Agreement.User, address common.Address) (string, common.Address, string, string, *big.Int, error) {
	res, err := contract.GetUser(nil, address)
	if err != nil {
		log.Fatal(err)
	}
	return res.Name, res.People, res.Email, res.Sign, res.GoodsNum, nil
}
